...
const normalizeSeverity = (value: unknown): Severity => {
  ...
  if (v.includes('red') || v.includes('위험') || v.includes('차단') || v.includes('high'))
    return 'Red';
  if (
    v.includes('yellow') ||
    v.includes('주의') ||
...
const normalizeAnalysis = (value: unknown): AnalysisResult => {
  ...
  const recommendedCCs = uniqClean(Array.isArray(obj.recommendedCCs) ? obj.recommendedCCs : []);
...
const localAnalyze = (data: {
  ...
  if (subject.length < 3) {
    improvements.push('제목을 조금 더 구체적으로 작성해 주세요. (예: [요청] 견적 확인 / [공유] 회의록)');
    severity = maxSeverity(severity, 'Yellow');
  }

  const body = data.body.trim();
  if (body.length < 30) {
    improvements.push('본문이 너무 짧습니다. 요청/배경/기한/다음 액션을 한 문장씩 추가해 주세요.');
    severity = maxSeverity(severity, 'Yellow');
  }
  ...
  if (data.hasAttachment && !mentionedAttachment) {
    improvements.push('첨부파일이 있다면 본문에 첨부 설명(무엇/왜/버전)을 한 줄 추가하면 좋습니다.');
    severity = maxSeverity(severity, 'Yellow');
  }

  const piiSignals: Array<{ re: RegExp; msg: string; sev: Severity }> = [
    { re: /\b\d{6}-?\d{7}\b/, msg: '주민등록번호 형태가 감지되었습니다. 발송 전 마스킹을 권장합니다.', sev: 'Red' },
    {
      re: /\b(?:\d[ -]*?){13,16}\b/,
  ...
  const rudeSignals: Array<{ re: RegExp; suggestion: string }> = [
    { re: /(빨리|당장|즉시|ASAP|urgent)/i, suggestion: '긴급함을 표현할 때는 “가능한 일정”을 함께 제시해 주세요.' },
    { re: /(왜 아직|이게 말이|당연히)/i, suggestion: '상대의 책임을 단정하는 표현은 완곡하게 바꾸는 것을 권장합니다.' },
  ];
...
export const analyzeMail = async (data: {
  ...
  const run = (async (): Promise<AnalysisResult> => {
    ...
    try {
      ...
      const allowedCc = new Set(extractEmails(`${data.to} ${data.cc ?? ''} ${data.subject} ${data.body}`));
